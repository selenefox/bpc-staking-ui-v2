import {IEventData} from "@bpc/rc-javascript-sdk";
  ...
  const grid = useLocalGridStore<IHistoryData>(async (offset: number, limit: number): Promise<[IHistoryData[], boolean]> => {
    ...
    for (const record of history) {
      ...
      if ('delegation' in record) {
        type = 'Delegation';
        amount = new BigNumber(record.delegation?.amount ?? '0').dividedBy(10 ** 18).toFixed(4);
      ...
      if ('undelegation' in record) {
        type = 'Undelegation';
        amount = new BigNumber(record.undelegation?.amount ?? '0').dividedBy(10 ** 18).toFixed(4);
      ...
      if ('claim' in record) {
        type = 'Claim';
        amount = new BigNumber(record.claim?.amount ?? '0').dividedBy(10 ** 18).toFixed(4);
