...
export class KeyProvider implements IKeyProvider {
  ...
  public async getChainConfig(): Promise<IChainConfig> {
    const [
      ...
      minStakingAmount,
      maxDelegateTotalAmount,
    ] = await Promise.all([
      this.chainConfigContract!.methods.getActiveValidatorsLength().call(),
      this.chainConfigContract!.methods.getCandidateLength().call(),
      ...
      this.chainConfigContract!.methods.getMaxDelegateTotalAmount().call(),
    ])
    return {
      activeValidatorsLength,
      candidateLength,
      ...
      maxDelegateTotalAmount: new BigNumber(minStakingAmount).dividedBy(10 ** 18),
    };
  }
